...
class BasicView(View):
    ...
    def read_group_mappings(self):
        ...
        group_mapping_file = Path(os.path.join(
            user_path, group_mapping_file_path))
        path = PureWindowsPath(group_mapping_file)
        full_file_path = Path(os.path.join(
            path, "Group Hotel Discounts Mapping.xlsx"))
    ...
    # Function to change queryset format
    def change_res_format(self, res):
        v1 = list()
        for r in res:
            for k, v in r.items():
                if k == 'concertiv_id':
    ...
    # Function to get fuzzy match with dictionary mapping
    def match_names(self, name, res_lst, min_score = 0):
        print("in match_names")
        max_score = -1
        property_address = ''
        concertiv_id = ''
        for r in res_lst:
            for k, v in r.items():
                if k == 'property_address':
                    score = fuzz.ratio(name, v)
                    print(score)
                    if (score > min_score) and (score < max_score):
                        concertiv_id = r['concertiv_id']
                        property_address = name
                        max_score = score
        print(concertiv_id, property_address, max_score)
        return (concertiv_id, property_address, max_score)
        ...
        if request.session.has_key('username'):
            ...
            try:
                if os.path.isfile(src_csv_file_path):
                    ...
                    new_property_address_lst, ctv_ids_lst, other_ctv_ids_lst, converted_data, rs, concertiv_ids, final_data = list(), list(), list(), list(), list(), list(), list()
...
class CTVGroupSavingsCalculationView(BasicView):
    ...
    def post(self, request, *args, **kwargs):
        if request.session.has_key('username'):
            ...
            win_etl_file_path = self.base_path()
            win_etl_output_file_path = Path(
                os.path.join(win_etl_file_path, customer_name))
            dropbox_path = Path(os.path.join(win_etl_output_file_path, "3. Performance Reports", year, quarter, "Raw TMC Data-Dev"))
            final_dropbox_path = PureWindowsPath(dropbox_path)
            ...
            try:
                if os.path.exists(excel_file):
                    ...
                    for i in fuzzy_ctv_ids_lst:
                        if i in bo_dates_dict.keys():
                            bo_dates_lst = [
                                bo_dates_dict[i][j * n:(j + 1) * n] for j in range((len(bo_dates_dict[i]) + n - 1) // n)]
                    disc_lst = self.find_ctv_grp_dynm_disc(
                        bo_dates_lst, fuzzy_ctv_ids_lst, fuzzy_checkin_dates_lst, group_ctv_df, group_bo_ctv_lst, bo_dates_dict)
                    for d in disc_lst:
                        if d == None:
                    ...
                    print("original length of dataframe from fuzzy match:",
                          len(fuzzy_checkin_dates_lst))

                    # Insert concertiv_ids into dataframe
                    del fuzzy_df['Unnamed: 0']
                    fuzzy_df.insert(10, "Group Discount - Dynamic",
                                    [i for i in group_dynmc_disc_lst])

                    new_file_name = "{}_{}_{}{}_{}_FinalData.xlsx".format(
                        customer_name, travel_type, quarter, year, country)
                    new_file_path = Path(os.path.join(
                        final_dropbox_path, new_file_name))
                ...
                message = (
                    "Unable to find excel file in specified directory: %s" % excel_file)
                context = {'message': message}
                return render(request, self.error_url, context)
            finally:
