...
class EmployerDepartmentAccessPolicy(CustomAccessPolicy):
    statements = get_access_policy_statements("user_works_for_employer", "anyone_can_create")

    @classmethod
    def scope_queryset(cls, request: Request, queryset: QuerySet[EmployerDepartment]) -> QuerySet[EmployerDepartment]:
    ...
    def user_works_for_employer(
        ...
        employer_department: EmployerDepartment = view.get_object()
        if isinstance(request.user, AnonymousUser):
            return False
        elif isinstance(request.user, User):
            return bool(employer_department.employer.id == request.user.person.employer.id)
